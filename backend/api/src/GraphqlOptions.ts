...
const PROJECT_PATH = join(process.cwd(), 'src');
...
export default class GraphqlOptions implements GqlOptionsFactory {
  ...
  createGqlOptions(): GqlModuleOptions {
    // merge schemas
    const loadedFiles = loadFilesSync(join(PROJECT_PATH, './modules/**/*.gql'));
    const typeDefs = mergeTypeDefs(loadedFiles);
    writeFileSync(join(PROJECT_PATH, 'schema.gql'), print(typeDefs));

    return {
      typePaths: [join(PROJECT_PATH, './modules/**/*.gql')],
      typeDefs: [constraintDirectiveTypeDefs],
      installSubscriptionHandlers: true,
      definitions: {
        path: join(PROJECT_PATH, 'gql-types.ts'),
        outputAs: 'class',
